...
const MainlistPath = "../data/sub_pd-list_life.json";
fetch(MainlistPath)
  .then((response)=> response.json() )
  .then((response)=>{
    const dataArr = [...response];
    const dataArrLen = dataArr.length;
    const elMainPdWrap =  document.querySelector('.main_pd_wrap');


  //선택자 변수  
  const MainlistCode = `<h4 class="blind">main_product_name</h4>
  <div class="thumbnail"><div class="shop_bag_icon"></div>
</div><div class="description"><div class="main_pd_name"><a href="#">제품명</a><p class="sub_text">서브해시태그</p></div><div class="main_pd_price"><span class="main_pd_p_dc"><span class="blind">할인가</span>24,500원</span><span class="main_pd_per_price"><span class="blind">정가</span>30,500원</span><span class="main_pd_sale_price"><span class="blind">할인률</span>20&#37;</span></div></div>`

for( let i =0; i<dataArrLen; i++){
  const mkDivConBox = document.createElement('div');
  mkDivConBox.setAttribute('class','main_conbox');
  mkDivConBox.innerHTML = MainlistCode;
  elMainPdWrap.append(mkDivConBox);
}



function PdDataSet(n){
  let i = n;
  const elDivConBox = elMainPdWrap.querySelectorAll('.main_conbox');
  const elPdImg = elDivConBox[i].querySelector('.thumbnail');
  const elPdName = elDivConBox[i].querySelector('.main_pd_name a');
  const elPdSubTxt = elDivConBox[i].querySelector('.sub_text');
  elPdImg.style.backgroundImage = 'url('+dataArr[i].mainImg+')';
  elPdName.innerText = dataArr[i].mainPdname;
  elPdSubTxt.innerText = dataArr[i].contentText;
}
