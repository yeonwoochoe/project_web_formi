...
const deviceCheck = [
  {type:'mobile', check:mobileSize , code: '../html/temp/mob_header.html', script:'../js/src/rwd/mobile_header.js'},
  {type:'tablet', check:tabSize , code: '../html/temp/tablet_header.html', script:'../js/src/rwd/tablet_header.js'},
  {type:'pc', check:pcSize , code: '../html/temp/pc_header.html', script:'../js/src/rwd/pc_header.js'}
];
...
// 기본화면에서 체크 : 사이즈변화이전 기본형태에서 체크 미작성
  deviceCheck.forEach ((media,index)=>{  // 배열형식으로된내용 각각 수행하여 체크
...
deviceCheck.forEach ((media,index)=>{  // 배열형식으로된내용 각각 수행하여 체크
  const mediaQuery = window.matchMedia(media.check);  // type-insert
  mediaQuery.addEventListener('change',(e)=>{ // mediaQuery 변화시 체크
    if(e.matches){ 
      nowDevice =  deviceCheck[index].type; 
      console.log(nowDevice);
      fnDevice(index);
