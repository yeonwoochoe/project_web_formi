...
{  
  ...
  const path2 = '../src/rwd';
  const data = {
    mob     : path + 'mob_header.html',
    tablet  : path + 'tablet_header.html',
    pc      : path + 'pc_header.html',
    mobJs   : path2 + 'mobile_header.js',
    tabJs   : path2 + 'tablet_header.js',
    pcJs    : path2 + 'laptop_header.js'
  }
  ...
  const fnDeviceNav = (type, tScript)=>{ // html 타입, script 타입
    ...
    .then(element  =>elHeadWrap.innerHTML = element) // 불러온 html삽입
